...
<div class="popup-overlay" id="popup" style="display: none;">
		...
		<span class="close-btn" onclick="closePopup()">X</span>
            <?php if (!empty($adimgae)){?>
				...
				<h2>안내 사항</h2>
				<p>새해복 많이 받으세요!</p>
                 <p>현재 구매하기 혹은 하단 버튼이</P><p> 보이지 않는 분들은</p>
    <p>플레이스토어에서 앱 업데이트나</p> <p>삭제 후 다시 다운로드 받아주시면</p><p> 감사하겠습니다.</p>
	<p>(명절에는 택배가 운행되지 않습니다.)</p>
				<?
			}
				?>
            <div>
	...
	<div class="category-container-wrapper">
		<div class="category-container" id="category-container">
			<?php
			$categoryquery2 = "SELECT category.category_idx, category.category_name, COUNT(menu.menuidx) as category_count
	FROM category
	...
	if ($data != 'all') {
		...
		if ($categoryresult->num_rows > 0) {
			...
			$optionstr = "WHERE category = '$category_idx' ORDER BY category DESC";
		} else {
			// 해당하는 카테고리 정보가 없는 경우
			$optionstr = "WHERE 1=0"; // 아무 상품도 표시하지 않음
		}
	} else {
		// 사용자가 'all'을 선택한 경우
		// 모든 상품을 표시하기 위한 조건 설정
		$optionstr = "WHERE category >= 1 AND category <= 9999 ORDER BY idate DESC";
	}

	$count = 0;
	$query = "SELECT * FROM menu $optionstr";
	$result = mysqli_query($conn, $query);
		...
		<div class="box">
			<div class="new-product">
				<span class="<?= $data ?>">
					<?php
					if ($data == 'all') {
						$allquery = "SELECT COUNT(menuidx) FROM menu";
						$allresult = $conn->query($allquery);
						$row4 = mysqli_fetch_row($allresult);
						$count5 = $row4[0];
					?>
						<div>
							<div onclick="allGoods()" style="cursor:pointer;">전체 상품&nbsp;<span class="span1"><?= $count5 ?></span></div>
						</div>
					<?php } else {
						$categoryquery1 = "SELECT COUNT(menuidx) FROM menu WHERE category = $category_idx";
		...
		<div class="product-container">
			...
			?>
				...
				<?php if ($i % 2 == 0) : ?>
					...
					<div class="product-item" onclick="newurldo('<?= $row['menuidx'] ?>','<?= $row['mname'] ?>','<?= $useridx ?>')">
						<?php if ($img_11 == 0) { ?>
							<img src="<?= $hostaddress ?>/image/no.png">
						<?php } else { ?>
							<img src="<?= $img_11 ?>">
						<?php } ?>
						<div class="product-info">
							<div class="product-title"><?= $row['mname'] ?></div>
							<div class="product-price"><?= number_format($row['price']) ?> 원</div>
	...
	?>
		<div class="recom_div">
			<span style="font-size: 32px; margin-left: 3%;">기획전</span>
			<?php
			$planQuery = "SELECT * FROM menu ORDER BY idate DESC";
				...
				<div class="plan_table" onclick="newurldo('<?= $row['menuidx'] ?>','<?= $row['mname'] ?>','<?= $useridx ?>')">
					...
					<div class="product-info">
						...
						<div class="product-rating">
							...
							for ($k = 1; $k <= 5; $k++) {
								if ($k <= $average_rating) {
									echo '<img src="' . $hostaddress . '/image/1.png">'; // Filled Star image
								} else {
									echo '<img src="' . $hostaddress . '/image/2.png">'; // Empty Star image
								}
							}
							?>
							<div class="product-review-count">
...
function setCookie(name, value, days) {
    ...
    var showWarning = getCookie("showWarning");
    if (!showWarning) {
