...
<body>
    ...
    <input type="hidden" name="useridx" id="useridx" value="<?=$useridx?>">
        ...
        <div class="regular_div" id="content-to-capture"> <!-- 전체 div -->
            ...
            <input type="text" class="r_name" id="name2" name="name" value ="<?=$name2?>" placeholder="신청인 이름" oninput="name_input(this)"> <!-- 신청자이름 -->

            <input type="text" class="r_birth" id="birthdate" name="birth" value="<?=$birthdate?>" placeholder="생년월일" oninput="birth_input(this)"> <!-- 신청자 생년월일 -->

            </div><!-- 신청자 이름 생년월일 div -->

            <div class="address_div"> <!-- 주소 div -->

                <input type="text" class="r_address" id="n_address" name="n_address" placeholder="신청인 주소"  readonly > <!-- 도로명주소 -->
            ...
            <div class="r_phone_div"> <!-- 신청인 휴대폰,이메일 div -->
                ...
                <input type="button" class="mycheck" value="본인인증" onclick="aaa()">
                <input type="text" class="r_email" id="email" name="email"  placeholder="이메일" oninput="email_input(this)"> <!-- 이메일 -->
            ...
            <div class="method_div"> <!-- 결제 방식 div -->
                ...
                <input type="checkbox" class="method1" name="pay_method_1" id="pay_method_1" value="1"> <!-- 자동 이체 매월 25일(CMS 월납) -->
            ...
            <div class="account_div"> <!--≈결제번호(계좌,통신번호) div -->
                ...
                <input type="button" class="account_check" value="계좌인증" onclick="account_check()">
            ...
            <div class="sign_div">
                <input type="text" class="s_name" id="name3" name="name3" placeholder="신청인 이름" readonly>
...
<img id="someImageElement" src="" alt="Captured image will appear here" />
    ...
    <script language="javascript">
        ...
        input.value = input.value.replace(/[^ㄱ-ㅎ가-힣a-zA-Z]/g, '');
    }
    ...
    function position_input(input) {
        // 입력된 값에서 영어 또는 한글이 아닌 문자를 제거합니다.
        input.value = input.value.replace(/[^ㄱ-ㅎ가-힣]/g, '');
    }
        ...
        function aaa(){
            ...
            var url=`./CMSSendSMS.php?receiver=${telnum}`;
                fetch(url).then(function(response){ 
                    ...
                    var msg="seterror</>"+ms;
                    viewinfo(msg);
                    var url = `<?=$hostaddress?>/CMS/cms_sms_popup.php?otp_num=${otp_num}`;
    ...
    function account_check(){
                ...
                fetch(url).then(function(response){ 
                    ...
                    if(topics.dataBody.result_cd == '0000')
                    {
                        ...
                        var msg="seterror</>"+ms;
		                viewinfo(msg);
                        var url = `<?=$hostaddress?>/CMS/cms_popup.php?request_no=${request_no}&res_uniq_id=${res_uniq_id}`;
                        var msg="popupView</>"+url;
		                viewinfo(msg);             
                    }
                    if(topics.dataBody.err_msg){
                        var err_msg = topics.dataBody.err_msg;
                        var msg="seterror</>"+err_msg;
		                viewinfo(msg);
                    }
                })
    ...
    function gojuso(){
            var url = `<?=$hostaddress?>/CMS/cms_jusoPopup.php`;
            var msg="popupView</>"+url;
            viewinfo(msg);  
        }
    </script>    
...
<script language="javascript">
    ...
    var url = "<?=$hostaddress?>/CMS/sign_popup2.php";
    var msg="loadwebview</>" + url + "</>" + idx + "</>" +mname+ "</>"+ useridx;
    viewinfo(msg);
}
function pop_up(idx,mname,useridx) {
    var url = "<?=$hostaddress?>/CMS/sign_popup.php";
    var msg="loadwebview</>" + url + "</>" + idx + "</>" +mname+ "</>"+ useridx;
    viewinfo(msg);
}
...
function onnext() {
    ...
    if(input_c_name == "")
    {
        var ms = "주민번호 앞자리를 입력해주세요";
        var msg="seterror</>"+ms;
        viewinfo(msg);
        return;
    }
    if(input_back_number == "")
    {
        var ms = "주민번호 뒷자리를 입력해주세요";
        var msg="seterror</>"+ms;
        viewinfo(msg);
        return;
    }
